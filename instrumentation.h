...
public:
  ...
  virtual RunResult Attach(int argc, char **argv, char *service_name, uint32_t init_timeout, uint32_t timeout) = 0;

  virtual RunResult AttachWithCrashAnalysis(int argc, char **argv, char *service_name, uint32_t init_timeout, uint32_t timeout) {
    return Attach(argc, argv, service_name, init_timeout, timeout);
  }
